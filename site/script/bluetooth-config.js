const laadData = function () {
    fetch(`${socketIP}/api/snakedata/bluetooth/getDevices`)
        ...
        .catch(function (error) {
            document.querySelector(".c-devices").innerHTML = "Fout bij het ophalen van devices."
...
const verwerkData = function (data) {
    if (data.length == 0) {
        ...
        for (let dev of data) {
            console.log(dev)
            document.querySelector(".c-devices").innerHTML += `<li class="c-devices__item" MAC="${dev["mac"]}">
            <div>
                ${dev['name']}
            </div>
...
const sendAddress = function (e) {
    ...
    let data = `{ "player": ${item.querySelector("select").value}, "mac": "${item.getAttribute("MAC")}" }`
    console.log(data);
    handleData(`${socketIP}/api/snakedata/bluetooth/getDevices`, function (data) {
